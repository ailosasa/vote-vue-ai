<template>
  <div class="app-container">
    ...
    <div class="vote-form">
      <div class="form-tip">✅ 专业技术人员：4项合计=100分 | ✅ 一般管理人员：10项合计=100分 | 🚫 同一IP仅可提交1次</div>
      ...
      <div class="section">
        ...
        <div v-for="name in data.technicalStaff" :key="name" class="person-box">
          ...
          <div class="score-items">
            <div>职业道德<input v-model.number="tech[name].moral" @input="calcTech(name)" min="0" max="100"></div>
            <div>工作作风<input v-model.number="tech[name].work_style" @input="calcTech(name)" min="0" max="100"></div>
            <div>担当作为<input v-model.number="tech[name].responsibility" @input="calcTech(name)" min="0" max="100"></div>
            <div>廉洁自律<input v-model.number="tech[name].integrity" @input="calcTech(name)" min="0" max="100"></div>
          </div>
      ...
      <div class="section">
        ...
        <div v-for="name in data.managementStaff" :key="name" class="person-box">
          ...
          <div class="score-items grid-10">
            <div>政治能力<input v-model.number="manage[name].political_ability" @input="calcManage(name)" min="0" max="100"></div>
            <div>政治表现<input v-model.number="manage[name].political_performance" @input="calcManage(name)" min="0" max="100"></div>
            <div>党建责任<input v-model.number="manage[name].party_duty" @input="calcManage(name)" min="0" max="100"></div>
            <div>专业素养<input v-model.number="manage[name].professional" @input="calcManage(name)" min="0" max="100"></div>
            <div>领导能力<input v-model.number="manage[name].leadership" @input="calcManage(name)" min="0" max="100"></div>
            <div>学习创新<input v-model.number="manage[name].innovation" @input="calcManage(name)" min="0" max="100"></div>
            <div>履职成效<input v-model.number="manage[name].performance" @input="calcManage(name)" min="0" max="100"></div>
            <div>担当作为<input v-model.number="manage[name].act" @input="calcManage(name)" min="0" max="100"></div>
            <div>作风形象<input v-model.number="manage[name].style_image" @input="calcManage(name)" min="0" max="100"></div>
            <div>廉洁从业<input v-model.number="manage[name].integrity_work" @input="calcManage(name)" min="0" max="100"></div>
          </div>
...
const submitAll = async () => {
  ...
  try {
    ...
    // 总分校验
    techPersons.forEach(n => { if (techTotal[n] !== 100) throw new Error(`${n} 技术人员总分必须=100`) })
    managePersons.forEach(n => { if (manageTotal[n] !== 100) throw new Error(`${n} 管理人员总分必须=100`) })
